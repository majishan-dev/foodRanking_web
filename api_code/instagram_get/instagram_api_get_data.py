...
def search_top_hash_tag_posts(business_id, hash_tag_id, access_token):
    ...
    url = f'https://graph.facebook.com/{hash_tag_id}/recent_media?user_id={business_id}&fields=caption,comments_count,id,like_count,media_type,media_url,permalink,timestamp,children%7Bmedia_url%7D&access_token={access_token}&limit=35'
    try:
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()
